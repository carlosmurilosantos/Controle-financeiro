...
abstract class RestController extends MY_Controller {
    ...
    public function __construct($config = 'rest')
    {
        ...
        // a abordagem original acima (carregamento de arquivo de configura��o)
        // foi substitu�da pelo uso da classe RestConfig criada para permitir
		// modulariza��o da API Rest
        // @author Reginaldo do Prado
        $this->load->library('rest/RestConfig');
        ...
        // a abordagem original acima (carregamento de arquivo em language/english)
        // foi substitu�da pelo uso da classe RestLang criada para permitir
        // modulariza��o da API Rest
        // @author Reginaldo do Prado
        $this->load->library('rest/RestLang');
    ...
    protected function _auth_override_check()
    {
        ...
        if (!empty($auth_override_class_method))
        {
            ...
            if (!empty($auth_override_class_method[$this->router->class]['*'])) // Check for class overrides
            {
                ...
                // Basic auth override found, prepare private
                if ($auth_override_class_method[$this->router->class]['*'] === 'private')
                {
                    $this->_prepare_basic_auth();
            ...
            if (!empty($auth_override_class_method[$this->router->class][$this->router->method]))
            {
                ...
                // Basic auth override found, prepare private
                if ($auth_override_class_method[$this->router->class][$this->router->method] === 'private')
                {
                    $this->_prepare_basic_auth();
        ...
        if (!empty($auth_override_class_method_http))
        {
            ...
            if(!empty($auth_override_class_method_http[$this->router->class]['*'][$this->request->method]))
            {
                ...
                // Basic auth override found, prepare private
                if ($auth_override_class_method_http[$this->router->class]['*'][$this->request->method] === 'private')
                {
                    $this->_prepare_basic_auth();
            ...
            if(!empty($auth_override_class_method_http[$this->router->class][$this->router->method][$this->request->method]))
            {
                ...
                // Basic auth override found, prepare private
                if ($auth_override_class_method_http[$this->router->class][$this->router->method][$this->request->method] === 'private')
                {
                    $this->_prepare_basic_auth();
    ...
    /**
     * Prepares for private authentication
     *
     * @access protected
     * @return void
     */
    protected function _prepare_basic_auth()
    {
        ...
        {
            // If the authentication header is set as private, then extract the username and password from
            // HTTP_AUTHORIZATION e.g. my_username:my_password. This is passed in the .htaccess file
            if (strpos(strtolower($http_auth), 'private') === 0)
            {
    ...
    protected function _force_login($nonce = '')
    {
        ...
        if (strtolower($rest_auth) === 'private')
        {
